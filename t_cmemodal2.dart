...
class t_cmemodal2 extends StatefulWidget {
  const t_cmemodal2({Key? key}) : super(key: key);

  @override
  _t_cmemodal2State createState() => _t_cmemodal2State();
}

class _t_cmemodal2State extends State<t_cmemodal2> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('http://arizshad-002-site18.atempurl.com/api/TotalHours'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': accessToken!,
        },
        body: jsonEncode(
            {'RegistrationId': userId, 'LicenseId': 2, 'FinId': '2'}),
      );
